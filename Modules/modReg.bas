...
Public Sub SetAutoRun(mode As Boolean)
    If mode = True Then
        SetStringKey HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion\Run", "Yippee Messenger", App.path & "\" & App.EXEName & ".exe"
    Else
        RemoveKey HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion\Run\", "Yippee Messenger"
    End If
End Sub

Public Function GetAutoRun() As Boolean
    GetAutoRun = GetStringKey(HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion\Run", "Yippee Messenger") <> ""
End Function
